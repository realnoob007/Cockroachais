...
def register():
    try:
        ...
        if config.REGISTRATION_METHOD == "captcha":
            captcha_id = request.json.get('captcha_id')  # 从请求中获取验证码ID
            user_captcha = request.json.get('code')     # 用户输入的验证码

            if not captcha_id or not user_captcha:
                return jsonify({'message': 'Captcha ID or code is missing'}), 400

            stored_captcha = r.get(captcha_id)
            if not stored_captcha or user_captcha.lower() != stored_captcha.decode().lower():
...
def get_captcha():
    ...
    captcha_id = str(uuid.uuid4())  # 生成唯一ID
    r.set(captcha_id, captcha_text)
    r.expire(captcha_id, 300)  # 设置有效期
    ...
    response.headers['Captcha-ID'] = captcha_id  # 设置响应头
    return response
